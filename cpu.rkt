...
(define ROM-NAME "C:\\Users\\david\\Downloads\\11_Scroll_V-master\\11_Scroll_V-master\\BUILD\\scroll_v.nes")
;(define ROM-NAME "C:\\Users\\david\\Downloads\\02_Hello2-master\\02_Hello2-master\\BUILD\\hello2.nes")
;(define ROM-NAME "d:\\tmp\\roms\\dk.nes")
;(define ROM-NAME "d:\\tmp\\roms\\Sicko Charlie (J).nes")
...
(define (reset-status-register)
  (C! #f) (Z! 0) (I! #f) (D! #t) (B! #f) (U! #t) (V! 0) (S! 0))
...
(define (C?)      C)     ; true, if the carry flag is set
(define (set-C)   (C! #t))
(define (clear-C) (C! #f))
...
(define halt-cycle 0)
(define (halt-cycle--) (when (> halt-cycle 0) (set! halt-cycle (sub1 halt-cycle))))
(define (halt-cycle+ c) (set! halt-cycle (+ halt-cycle c)))
;;;
;;; MEMORY
;;;
...
(define (ppu-reg-ref i)
  ;(displayln (~a "ppu-reg-ref " i " " (bin (vector-ref  ppu-registers i))))
  (vector-ref  ppu-registers i))
(define (ppu-reg-set! i b)
  (case i
    [(7)   (ppu-data-write  b)]  ; $2007  ; fast path
    [(0)   ;(displayln (list 'ppu-ctrl (bin b)) (current-error-port))
           (when (and (not (ppu-ctrl-V?)) (bitand #x80 b) (ppu-status-V?))
             (set! nmi-due-to-vblank? #t)
             (displayln "generate NMI!!!")
             )
...
(define (load a) ; retrieve the byte value stored at address a
  ...
  (cond
    ...
    [(<= 4 bank 7) (begin0 (ppu-reg-ref     (word-and a #b111))
                           (when (= a #x2004) (displayln "read $2004"))
                           (when (= a #x2002) (ppu-status-V-clear) ))]   ; index of PPU register = lower 3 bits
    [(= bank 8)    (cond
                     [(= a #x4016) (read-joy a) #;(error 'load "joy 0")]
                     [(= a #x4017) 0]     ; JOY 1 ; XXX TODO
                     [else       #;(displayln (~a "read " (bin a)))  0])]   ; we will ignore the APU for the time being
    [else          (error 'load "unexpected ")]))          ; no other banks need special treatment

(define (store a b) ; store the byte value b at address a
  ;(when (= a #x200) #;(show-dis) (displayln (~a "store $200: " b)))
  ...
  (cond
    ...
    [(= bank 8)    (cond
                     [(= a #x4016) (write-joy a b)]
                     [(= a #x4014) (ppu-oam-dma-write b)]
                     [else         (void)])]
    [else          (error 'store "unexpected ")]))

(define (load2 a)    (word (load                    (+ a 1))  (load a)))
;(define (load2bug a)    (word (load                    (+ a 1))  (load a)))
...
Joy Controller
|#
(define joy1-strobe-state 0)
(define joypad-last-write 0)
(define joy1-states (make-vector 8 #x40))
(define (write-joy a b)
  (when (= a #x4016)
    (when (and (= b 0) (= joypad-last-write 1)) (set! joy1-strobe-state 0))
    (set! joypad-last-write b)
    ))
(define (read-joy a)
  (define val 0)
  (cond
    [(< joy1-strobe-state 8) (set! val (vector-ref joy1-states joy1-strobe-state))]
    [(= joy1-strobe-state 19) (set! val 1)]
    [else (set! val 0)])
  (set! joy1-strobe-state (add1 joy1-strobe-state))
  (when (= joy1-strobe-state 24) (set! joy1-strobe-state 0))
  ;(displayln (~a "read joy val:" val))
  val)
;;;
...
(define (ppu-oam-dma-write v)
  (halt-cycle+ 513)
  ;(displayln (~a "oam at " (hex (* v #x100))))
  (let ((base-addr (* v #x100)))
    (for ((i 256))
      (vector-set! ppu-oam i (load (+ base-addr i))))
    )
  )
...
(define SHOW-DIS #f)
(define (show-dis) (set! SHOW-DIS #t)(set! trace-fuel 500))
(define (step)
  ...
  (when (and SHOW-DIS (> trace-fuel 0)) (set! trace-fuel (- trace-fuel 1)) (displayln (disassemble-single PC)))
...
(define (run start-address)
  (with-handlers ([(thunk* #t) (λ(e) (displayln e))])
...
(define-instruction (tsx _)                     (X! SP) (S! X) (Z! X))
(define-instruction (pha _)                                                  (push! A))
;(define-instruction (txs _)                                                  (push! X))
(define-instruction (txs _)                                                  (SP! X))
...
(define-instruction (cmp a) (let ([t (- A (load a))][v (byte-    A (load a))]) (C! (if (>= t 0) #t #f)) (S! v) (Z! v)))
(define-instruction (cpx a)
  (let ([t (- X (load a))][v (byte-    X (load a))])
    (C! (if (>= t 0) #t #f))
    (S! v)
    (Z! v)
  ))

(define-instruction (cpy a) (let ([t (- Y (load a))][v (byte-    Y (load a))])    (C! (if (>= t 0) #t #f)) (S! v) (Z! v)))
...
(define (adc-body v)
  ...
  (let* ([c (if (C?) 1 0)]
    ...
    #;(V! (if (or (and (byte-neg? A) (byte-neg? v) (byte-pos? s))
            ...
            1 0))
    (V! (if (and (= 0 (byte-and (byte-xor A v) #x80))
                (not (= 0 (byte-and (byte-xor A t) #x80))))
            1 0))
    (A! s)
    (Z! s)
    (S! t)))
...
(define (oam-dump [a #x0] [lines 8])
  ;(define (byte a)   (hex (load (word-and a #xFFFF))))
  (define (byte a)   (hex (vector-ref ppu-oam a)))
  (define (bytes4 a) (~a (byte a) " " (byte (+ a 1)) " " (byte (+ a 2)) " " (byte (+ a 3)) " "))
  (define (printable? c) (regexp-match #px"[ -~]" (string c)))
  (define (ch x) (let ([c (integer->char x)]) (if (printable? c) c #\.)))
  (define (str a) (apply string (for/list ([x 16]) (ch (load (word-and (+ a x) #xFFFF))))))
...
(define (read-ines-file path)
  ;(with-input-from-file (build-path ROOT path)
      ...
      (let loop ([cycles cycles]) ; cycles to spend
        (cond
          ...
          [(= state 0)                    ; (display "START FRAME" (current-error-port))
                                          ...
                                          (when (generate-nmi-at-start-of-vblank?)
          ...
          [(= state 1) ;(display "1")
                       (cond
                         ...
                         [else            (define new-cycles (- cycles left))    ; use left cycles
                                          (ppu-status-V-clear)                     ; vblank stops
...
(define colors-as-rgb
  #(#( 117  117  117) #(  39  27 143) #(  0  0 171) #( 71   0 159)
    ...
    #(188 188 188) #(  0  115 239) #( 35  39 239) #( 131  0 243)
...
(define (render-line-sprite y)
  (for ((i 64))
    (let* ((offset (* i 4))
           (spr-y (vector-ref ppu-oam offset))
           (spr-indx (vector-ref ppu-oam (+ offset 1)))
           (spr-attr (vector-ref ppu-oam (+ offset 2)))
           (spr-x (vector-ref ppu-oam (+ offset 3))))
      (when (= spr-y y)
        ;(displayln "ok")
        (plot-pixel/screen-buffer spr-x spr-y #x80)
        ))))
...
(define (read-hero-rom)     (read-ines-file "roms/hero.nes"))
;;; Uncomment to try
(require pict)


;(define hero-rom (read-hero-rom))
(define dk-rom (read-ines-file ROM-NAME))
...
(define ROM dk-rom)
;(scale (bitmap (chr-rom->image (ines-chr-rom ROM))) 3)
(ines-mapper ROM)
(ines-mirroring-type ROM)
(install-rom ROM)
;(install-rom bg-rom)
;(install-rom gt-rom)
; (install-rom bf-rom)
...
(define my-canvas%
  (class canvas% 
    (define/override (on-char key-event)
      (cond
        [(equal? (send key-event get-key-code) 'up) (vector-set! joy1-states 4 #x41)]
        [(equal? (send key-event get-key-release-code) 'up) (vector-set! joy1-states 4 #x40)]
        [(equal? (send key-event get-key-code) 'down) (vector-set! joy1-states 5 #x41)]
        [(equal? (send key-event get-key-release-code) 'down) (vector-set! joy1-states 5 #x40)]
        [(equal? (send key-event get-key-code) 'left) (vector-set! joy1-states 6 #x41)]
        [(equal? (send key-event get-key-release-code) 'left) (vector-set! joy1-states 6 #x40)]
        [(equal? (send key-event get-key-code) 'right) (vector-set! joy1-states 7 #x41)]
        [(equal? (send key-event get-key-release-code) 'right) (vector-set! joy1-states 7 #x40)]
        [(equal? (send key-event get-key-code) #\a) (vector-set! joy1-states 0 #x41)]
        [(equal? (send key-event get-key-release-code) #\a) (vector-set! joy1-states 0 #x40)]
        [(equal? (send key-event get-key-code) #\b) (vector-set! joy1-states 1 #x41)]
        [(equal? (send key-event get-key-release-code) #\b) (vector-set! joy1-states 1 #x40)]
        [(equal? (send key-event get-key-code) #\space) (vector-set! joy1-states 2 #x41)]
        [(equal? (send key-event get-key-release-code) #\space) (vector-set! joy1-states 2 #x40)]
        [(equal? (send key-event get-key-code) #\return) (vector-set! joy1-states 3 #x41)]
        [(equal? (send key-event get-key-release-code) #\return) (vector-set! joy1-states 3 #x40)]
...
#;(begin (install-opcodes-from-assembler
        (assembler (append test-code '((STOP))))
        )
       (run #x0600)
       (displayln (mem-dump 0))
       )
(define (cpu-dump x) (displayln (mem-dump x)))
#;(define my-code (assembler (append test-code '((STOP)))))
(cpu-dump #x80c)
(define (stop)  (kill-thread  the-process))
(define (resume)  (thread-resume  the-process))
(define (suspend) (thread-suspend the-process))
(define the-process (thread (λ () (run PC))))
